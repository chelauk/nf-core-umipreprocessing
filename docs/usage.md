# nf-core/umipreprocessing: Usage

## :warning: Please read this documentation on the nf-core website: [https://nf-co.re/umipreprocessing/usage](https://nf-co.re/umipreprocessing/usage)
...
```console
nextflow run nf-core/umipreprocessing --input samplesheet.csv --genome GRCh37 -profile docker
```
...
```console
nextflow pull nf-core/umipreprocessing
```
...
First, go to the [nf-core/umipreprocessing releases page](https://github.com/nf-core/umipreprocessing/releases) and find the latest version number - numeric only (eg. `1.3.1`). Then specify this when running the pipeline with `-r` (one hyphen) - eg. `-r 1.3.1`.
